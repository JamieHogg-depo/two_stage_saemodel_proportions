data {
	...
	real<lower=0> prior_scale_res; 			// standard error for prior on residual sigma
}
transformed data{
	// DECLARACTIONS
...
parameters {
	...
	vector[m] Z_e;								// standard normal
	vector[n] Z_res;							// standard normal
	real<lower=0> sigma_e;						// scale for area-random effect
	real<lower=0> sigma_res;					// scale for residuals
}
transformed parameters{
	vector[n] gmma;
	for(i in 1:n){
		gmma[i] = intercept_u + x_u[i,] * beta_u + Z_e[ps_area[i]] * sigma_e + Z_res[i] * sigma_res;
	}
}
model{
	// PRIORS
		...
		target += normal_lpdf( sigma_res | 0, prior_scale_res );
